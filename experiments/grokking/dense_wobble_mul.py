"""
Dense step-to-step analysis for mul seed 0 — cross-task check
on the drift-not-hopping claim established for sub seeds 1/2.

Same post-grok window (steps 11000-30000) for comparability.
mul grok crossing was at step 3000 for this seed, so 11k is
solidly post-grok.
"""
import torch, re
...
RUN = Path(f"/home/nate-agx/chronicle/experiments/grokking/runs/v2_mul_seed{SEED}/snapshots")
OUT = Path(f"/home/nate-agx/chronicle/experiments/grokking/dense_wobble_mul_seed{SEED}.csv")
...
print(f"mul seed 0: analyzing {len(window)} snapshots from step {window[0]} to {window[-1]}")
