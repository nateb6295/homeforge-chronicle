#!/usr/bin/env python3
"""2-layer addition at p=97. Tests whether:
  - the concentration tail still sits in embeddings + attn biases, or
  - with more layers the computation distributes and the tail flattens.

Same task as grok_v2 (mod-97 addition), same hyperparams, same training
budget — only the architecture changes to 2 layers.
"""
import argparse
import json
import time
...
class TwoLayer(nn.Module):
    def __init__(self, p=P, d_model=128, n_heads=4):
        ...
        self.block0 = Block(d_model, n_heads)
        self.block1 = Block(d_model, n_heads)
...
def run(tag, steps, seed):
    ...
    print(f"device: {device}  task: 2-layer add mod {P}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", default=f"v2_2layer_seed{SEED}")
