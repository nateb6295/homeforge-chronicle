#!/usr/bin/env python3
"""
Grokking v2 — with weight snapshots.

Same task as v1 (mod-97 addition, 1-layer transformer, AdamW wd=1.0).
Adds: save full state_dict every LOG_EVERY steps so post-hoc analysis can
answer the wobble question:

  H1 (flatness-seeking): at wobble-troughs, weights are briefly perturbed
  but return to the same basin. Weight distance from pre-wobble → small.
  Representations (Fourier features) stay fixed.

  H2 (alt-algorithm): at wobble-troughs, weights begin to occupy a
  different basin — possibly a different algorithmic solution — and are
  pulled back. Weight distance from pre-wobble → larger; representations
  would show brief structural rearrangement.

Analysis plan (separate script):
  - Compute pairwise weight distance matrix across all snapshots
  - Flag wobble steps (val_acc < 0.99)
  - For each wobble: distance(wobble_trough, prev_stable) vs
    distance(random pair of stable snapshots) — if wobble distances are
    outliers, that's H2 evidence
  - Extract attention-output + MLP-output logits at each snapshot for a
    fixed validation batch; Fourier-decompose; see if frequency content
    shifts during wobbles
"""
...
def run(tag: str, steps: int, seed: int = SEED):
    ...
    for step in range(1, steps + 1):
        ...
        if step % LOG_EVERY == 0:
            ...
            # Save weights every log step
            torch.save(model.state_dict(),
                       snap_dir / f"step_{step:06d}.pt")

            # Save probe logits for fast post-hoc rep analysis
            model.eval()
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", default=f"v2_{int(time.time())}")
