#!/usr/bin/env python3
"""
Evolutionary Model Forge — Action Compliance LoRA Training.

Trains a LoRA adapter focused on JSON action format compliance.
Near-copy of train_mind_lora.py with action-specific defaults:
  - max-seq-len 1024 (cycles are longer than capsule Q&A)
  - Input: ~/action_training.jsonl
  - Output: ~/action-lora-adapter/
  - Checkpoints: /mnt/hdd/forge/checkpoints-action/

Runs on AGX Orin 64GB in fp16.

Usage:
    python3 /tmp/train_action_lora.py --pilot     # 200 samples, 1 epoch (~1h)
    python3 /tmp/train_action_lora.py              # full run (~2-4h)
    python3 /tmp/train_action_lora.py --resume     # resume from checkpoint
"""
...
def main():
    parser = argparse.ArgumentParser(description="Action compliance LoRA training for Chronicle Mind")
    parser.add_argument("--pilot", action="store_true", help="Use first 200 samples only")
    parser.add_argument("--epochs", type=int, default=1, help="Number of training epochs")
    parser.add_argument("--batch-size", type=int, default=2, help="Per-device batch size")
    parser.add_argument("--grad-accum", type=int, default=4, help="Gradient accumulation steps")
    parser.add_argument("--lr", type=float, default=2e-4, help="Learning rate")
    parser.add_argument("--rank", type=int, default=16, help="LoRA rank")
    parser.add_argument("--max-seq-len", type=int, default=1024, help="Max sequence length (longer for cycles)")
    parser.add_argument("--resume", action="store_true", help="Resume from latest checkpoint")
    parser.add_argument("--input", type=str, default=os.path.expanduser("~/action_training.jsonl"),
                        help="Training data JSONL path")
    parser.add_argument("--output", type=str, default=os.path.expanduser("~/action-lora-adapter"),
    ...
    if not os.path.isfile(args.input):
        ...
        print("Run format_action_data.py first.")
        sys.exit(1)
    ...
    # ── Training arguments ─────────────────────────────────────
    checkpoint_dir = "/mnt/hdd/forge/checkpoints-action"
    ...
    # ── Train ──────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"Starting ACTION LoRA {'pilot' if args.pilot else 'full'} training...")
    print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print(f"Action LoRA training complete!")
