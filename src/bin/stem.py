...
class Stem:
    ...
    def _parse_clusters(self, raw: str) -> list:
        ...
        # record { id = 148_498 : nat64; theme = "..."; updated_at = ... : nat64;
        #          capsule_ids = vec { 360 : nat64; ... }; strength = 1.0 : float32; }
        record_pattern = re.compile(
            r'record\s*\{[^}]*?'
            r'id\s*=\s*([\d_]+)\s*:\s*nat64.*?'
            r'theme\s*=\s*"([^"]*)".*?'
            r'capsule_ids\s*=\s*vec\s*\{([^}]*)\}.*?'
            r'strength\s*=\s*([\d.]+)\s*:\s*float32.*?'
            r'updated_at\s*=\s*([\d_]+)\s*:\s*nat64',
            re.DOTALL
        )
        # Also try with fields in the actual order from the canister
        for m in re.finditer(
            r'record\s*\{[^}]*?id\s*=\s*([\d_]+)\s*:\s*nat64;'
            r'\s*theme\s*=\s*"([^"]*?)";'
            r'\s*updated_at\s*=\s*([\d_]+)\s*:\s*nat64;'
            r'\s*capsule_ids\s*=\s*vec\s*\{([^}]*)\};'
            r'\s*strength\s*=\s*([\d.]+)\s*:\s*float32;',
            raw, re.DOTALL
        ):
