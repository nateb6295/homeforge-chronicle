...
def extract_relationships(db: DB, brief: str, original_text: str, source_type: str, source_id: int, timestamp: int):
    ...
    try:
        ...
        for rel in rels[:5]:
            ...
            # Reclassify related_to using evidence (Build #89)
            if relation == "related_to" and context and len(context) > 50:
                try:
                    from kg_reclassify import _best_predicate
                    better, _score = _best_predicate(context)
                    if better != "related_to":
                        relation = better
                except Exception:
                    pass
