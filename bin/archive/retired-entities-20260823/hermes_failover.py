#!/usr/bin/env python3
"""mistral_failover — safety net for Opus.

If Nate posts to #operator and Opus's last trace is older than STALE_THRESHOLD
seconds, this tags Mistral in #operator with Nate's message forwarded,
so Mistral can answer in place of Opus.
...
MISTRAL_USER_ID = "1469193641431138427"
...
def already_mentioned_mistral(content: str) -> bool:
    c = content.lower()
    return MISTRAL_USER_ID in content or "@mistral" in c or "@hermes" in c


def forward_to_mistral(env: dict, nate_msg: dict, trace_age: int) -> bool:
    ...
    inner = inner[:1500]
    body = (
        f"<@{MISTRAL_USER_ID}> — Opus hasn't written in {trace_age // 60}m. "
...
def main() -> int:
    ...
    if already_mentioned_mistral(nate["content"]):
    ...
    if forward_to_mistral(env, nate, trace_age):
        state["last_forwarded_created_at"] = nate["created_at"]
