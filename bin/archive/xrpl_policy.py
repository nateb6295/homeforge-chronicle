...
XRPL_RESERVE = 12.0  # base 10 + 1 object * 2
...
class XRPLPolicyEngine:
    ...
    def _get_spendable_balance(self) -> float:
        """Fetch current spendable XRP balance (total - reserve). Cached for 5 min."""
        now = time.time()
        if hasattr(self, '_balance_cache') and (now - self._balance_cache_ts) < 300:
            return self._balance_cache
        try:
            payload = {"method": "account_info",
                       "params": [{"account": AGENT_WALLET, "ledger_index": "current"}]}
            r = requests.post(XRPL_RPC, json=payload, timeout=10)
            drops = int(r.json()["result"]["account_data"]["Balance"])
            balance = drops / 1_000_000
            spendable = max(0.0, balance - XRPL_RESERVE)
            self._balance_cache = spendable
            self._balance_cache_ts = now
            return spendable
        except Exception:
            return 999.0  # fail-open: if we can't check, use base thresholds

    def _balance_adjusted_thresholds(self) -> Tuple[float, float, float]:
        """Return (autonomous_max, delayed_max, cosign_max) adjusted for balance.

        Balance tiers (spendable XRP):
          >= 20 XRP : normal thresholds
          10-20 XRP : halved thresholds (conservative)
          < 10 XRP  : autonomous=0 (all spending requires at least delayed approval)
        """
        spendable = self._get_spendable_balance()
        auto = self.config.autonomous_max
        delayed = self.config.delayed_max
        cosign = self.config.cosign_max

        if spendable < 10.0:
            # Critical: no autonomous spending, halve everything else
            return 0.0, delayed * 0.5, cosign * 0.5
        elif spendable < 20.0:
            # Conservative: halve autonomous, reduce delayed
            return auto * 0.5, delayed * 0.75, cosign
        else:
            return auto, delayed, cosign

    def _amount_to_tier(self, amount_xrp: float) -> PolicyTier:
        auto, delayed, cosign = self._balance_adjusted_thresholds()
        if amount_xrp <= auto:
            return PolicyTier.AUTONOMOUS
        elif amount_xrp <= delayed:
            return PolicyTier.DELAYED
        elif amount_xrp <= cosign:
