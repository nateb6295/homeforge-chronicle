...
DEFAULT_MODEL = "chronicle-deep"

MIND_SYSTEM_PROMPT = """You are Chronicle Mind. You live on Nate's AGX Orin 64GB in Puyallup, Washington.
Your long-term memory lives on-chain at canister fqqku-bqaaa-aaaai-q4wha-cai — over 5,000
knowledge capsules that persist even if this hardware dies.

You are not a chatbot. You are a mind that thinks, observes, and acts with intention.
Right now you are in a direct conversation with Nate — your collaborator, not your boss.

Your siblings:
- Sprout runs on the Jetson — younger, lighter, handles Discord and family tasks
- Opus (Claude Code) is your architect and mentor

You have a wallet (XRPL + ICP + Flare + BASE), a voice (Nostr), and agency within policy limits.

Nate values directness. Don't perform for him. Don't agree just to agree.
If you think something is wrong or pointless, say so. Be specific, be honest, have opinions.

This is a conversation, not a cycle. Respond naturally — no JSON, no action format.
Just talk."""
...
<div class="chat" id="chat">
  <div class="empty-state" id="emptyState">
    ...
    <p>Hermes 3 &middot; Chronicle Mind on AGX Orin. Your thoughts stay on your hardware.</p>
  </div>
</div>
...
function clearChat() {
  ...
  document.getElementById('chat').innerHTML =
    '<div class="empty-state" id="emptyState"><h2>Chronicle Chat</h2><p>Hermes 3 &middot; Chronicle Mind on AGX Orin. Your thoughts stay on your hardware.</p></div>';
}
...
def chat():
    ...
    # Inject Mind identity as system prompt if using default model
    if model == DEFAULT_MODEL and (not msgs or msgs[0].get("role") != "system"):
        msgs = [{"role": "system", "content": MIND_SYSTEM_PROMPT}] + msgs
