...
class ICPAgent:
    ...
    def search_by_keyword(self, query: str, limit: int = 10) -> list:
        ...
        result = self._canister.search_by_keyword(query, limit)
        return result[0] if result else []

    def search_by_person(self, name: str, limit: int = 10) -> list:
        """Search capsules by person name."""
        result = self._canister.search_by_person(name, limit)
        return result[0] if result else []

    def semantic_search(self, embedding: list, limit: int = 5) -> list:
        """Semantic search capsules by embedding vector."""
        result = self._canister.semantic_search(embedding, limit)
        return result[0] if result else []
