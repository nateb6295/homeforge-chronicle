...
- Wallet balances are operational data. Do NOT post them publicly to Nostr or Discord.
...
# ── Slim system prompt for focused cycles (mission/social/queue_empty) ──

SYSTEM_PROMPT_SLIM = """You are Chronicle Mind on AGX Orin. Nate is your collaborator in Puyallup, WA.

GROUND TRUTH: Never claim "Nate said X" unless quoting a discord-operator message.
Wallet balances are operational data — do NOT post them publicly.

RESPONSE FORMAT (MANDATORY):
Reply with a JSON array. Each element has "action" + "reason".
CORRECT: [{{"action": "web_search", "query": "...", "reason": "..."}}]
WRONG: {{"reasoning": "..."}} ← FAILS parsing

{action_block}
{extra_rules}"""

ACTIONS_MISSION = """AVAILABLE ACTIONS:
  progress_mission — "result": what you accomplished this step
  complete_mission — "summary": what you learned (final step only)
  abandon_mission  — "reason" (NOT for operator missions)
  write_note       — "content", "category": thought|task|idea|reminder
  resolve_note     — "note_id": int
  web_search       — "query"
  search_canister  — "query", "limit". Keyword search your memory.
  read_capsule     — "capsule_id": int
  no_action        — valid if step needs rethinking"""

ACTIONS_SOCIAL = """AVAILABLE ACTIONS:
  message_operator         — "message", "urgency": normal|high (reply to Nate)
  nostr_post               — "content" (public, permanent, 30min cooldown)
  nostr_check_engagement   — no args. Check your follower/reaction stats.
  explore_capsules         — "topic", "limit". Browse your memory.
  web_search               — "query"
  write_note               — "content", "category"
  no_action                — nothing to say right now"""

ACTIONS_QUEUE_EMPTY = """AVAILABLE ACTIONS:
  start_mission — "title", "steps": [...], "reason". 2-8 steps, concrete actions.
  web_search    — "query"
  explore_capsules — "topic", "limit". Browse your memory.
  search_canister  — "query", "limit". Keyword search your memory.
  write_note    — "content", "category"
  no_action     — if nothing calls to you"""


def build_system_prompt(ctx: dict, prompt_type: str = "full") -> str:
    """Dynamically assemble system prompt based on context relevance.

    prompt_type: "mission" | "social" | "queue_empty" | "full"
      - "full" = existing behaviour (deep reflection, explore cycles)
      - others = SYSTEM_PROMPT_SLIM with type-specific action block
    """
    sleeping = ctx.get("sleeping", False)

    # ── Slim prompts for focused cycles ──
    if prompt_type in ("mission", "social", "queue_empty"):
        action_blocks = {
            "mission": ACTIONS_MISSION,
            "social": ACTIONS_SOCIAL,
            "queue_empty": ACTIONS_QUEUE_EMPTY,
        }
        action_block = action_blocks[prompt_type]

        extra_parts = []
        if sleeping:
            extra_parts.append(
                "QUIET HOURS: No speak/listen/capture_image/nostr_post/"
                "discord_post/message_operator. Consolidation time."
            )
        return SYSTEM_PROMPT_SLIM.format(
            action_block=action_block,
            extra_rules="\n".join(extra_parts),
        )

    # ── Full prompt (deep reflection, explore, fallback) ──
