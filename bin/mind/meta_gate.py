...
def meta_gate_layer1(db, proposed: List[Dict], window: int = 6) -> Tuple[str, str]:
    ...
    # Check 2b: Proposed action streak — what the model WANTED to do (pre-redirect)
    # Lower threshold (3) since proposed = true intent even when execution is redirected
    proposed_history = db.query(
        "SELECT proposed_actions FROM thought_stream ORDER BY id DESC LIMIT ?", (window,)
    )
    if len(proposed_history) >= 3:
        for name in set(proposed_names):
            if name == "no_action":
                continue
            p_streak = 0
            for ph in proposed_history:
                try:
                    if name in json.loads(ph.get("proposed_actions", "[]")):
                        p_streak += 1
                    else:
                        break
                except Exception:
                    break
            if p_streak >= 3:
                return ("redirect", f"'{name}' PROPOSED in {p_streak} consecutive cycles (true intent loop)")
