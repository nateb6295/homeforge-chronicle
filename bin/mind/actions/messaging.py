...
_MIND_CHANNEL_ID = os.environ.get("MIND_CHANNEL_ID", "1478214472786251837")
_NATE_DISCORD_ID = os.environ.get("NATE_DISCORD_ID", "1469162268204728441")


def act_discord_post(mind, action: dict, cid: str) -> str:
    """Post to #mind Discord channel and ping Nate. Direct REST API — no bot service needed."""
    ...
    if not _DISCORD_TOKEN:
        return "false - SPROUT_DISCORD_TOKEN not set in environment"

    # Always ping Nate unless explicitly suppressed
    ping = action.get("ping", True)
    if ping:
        content = f"<@{_NATE_DISCORD_ID}> {content}"
    content = content[:1900]

    try:
        resp = requests.post(
            f"https://discord.com/api/v10/channels/{_MIND_CHANNEL_ID}/messages",
            headers={
        ...
        if resp.status_code in (200, 201):
            return f"true - Posted to #mind: {safe_truncate(content, 80)}"
        return f"false - Discord API {resp.status_code}: {resp.text[:200]}"
    except Exception as e:
