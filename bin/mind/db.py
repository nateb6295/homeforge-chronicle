...
class DB:
    def __init__(self, path: str):
        ...
        try:
            ...
            if "action_signatures" not in cols:
                ...
                self.conn.commit()
            if "proposed_actions" not in cols:
                self.conn.execute("ALTER TABLE thought_stream ADD COLUMN proposed_actions TEXT DEFAULT '[]'")
                self.conn.commit()
        except Exception:
    ...
    _proposed_actions_migrated = False

    def log_thought(self, cid: str, reasoning: str, context_summary: str, actions: str,
                    results: str = "", action_sigs: str = "", proposed_actions: str = "[]"):
        ...
        if not DB._action_results_migrated:
            ...
            if "proposed_actions" not in cols:
                self.run("ALTER TABLE thought_stream ADD COLUMN proposed_actions TEXT DEFAULT '[]'")
            DB._action_results_migrated = True
            DB._action_sigs_migrated = True
            DB._proposed_actions_migrated = True
        elif not DB._action_sigs_migrated:
            cols = [r["name"] for r in self.query("PRAGMA table_info(thought_stream)")]
            if "action_signatures" not in cols:
                self.run("ALTER TABLE thought_stream ADD COLUMN action_signatures TEXT DEFAULT ''")
            DB._action_sigs_migrated = True
        if not DB._proposed_actions_migrated:
            cols = [r["name"] for r in self.query("PRAGMA table_info(thought_stream)")]
            if "proposed_actions" not in cols:
                self.run("ALTER TABLE thought_stream ADD COLUMN proposed_actions TEXT DEFAULT '[]'")
            DB._proposed_actions_migrated = True
        self.run(
            ...
            "action_results, action_signatures, proposed_actions, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (cid, reasoning, context_summary, actions, results, action_sigs, proposed_actions, now_ts()),
        )
